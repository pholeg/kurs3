...
def main():
    """Формируем вывод по заданному шаблону. На входе dict из исходной выписки и dict c 5ю последними ID"""
        ...
        for trans in load_transactions():
            if not trans == {} and trans['id'] == value_:
                ...
                if 'from' in trans.keys():
                    print(f"{un_visible_number(trans['from'])} -> ", end='')
                if 'to' in trans.keys():
                    print(f"{un_visible_number(trans['to'])}")
